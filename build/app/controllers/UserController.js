  ...
  async store(req, res) {
    ...
    if (req.body.phone_login) {
      ...
      if (smsStatusCode === '00') {
        ...
        const { id, phone_login } = await _User2.default.create({
    ...
    if (req_url.includes('userspublic')) {
      ...
      const emailcode = _crypto2.default.randomBytes(6).toString('hex');
      const app = req.body.app ? req.body.app : 'Senar';
    ...
    const { id, name, email, phone1, phone2, city, state } = await _User2.default.create(
      req.body
    );
  ...
  async update(req, res) {
    ...
    if (req.body.phone_login) {
      ...
      if (userExists) {
        ...
        const accessToken = _jsonwebtoken2.default.sign({ id: userExists.id }, _auth2.default.secret, {
          expiresIn: _auth2.default.expiresIn,
        });

        const accessTokenRefresh = _jsonwebtoken2.default.sign(
          { id: userExists.id },
          _auth2.default.secretRefresh
        );
